...
### Bad implementation
Manually copying fields is error-prone and tightly couples the client to the concrete class.
...
def copy_shape(original: Shape) -> Shape:
    # Bad: Coupled to concrete class. What about subclasses like Circle?
    copy = Shape(0, 0, "")
    copy.x = original.x
    copy.y = original.y
    copy.color = original.color
    # Private or dynamically-added attributes will be missed!
    return copy
```

### Better implementation (using `copy` module)
Python’s built-in `copy` module provides shallow and deep copy, but lacks a clean protocol for customization.
...
# Shallow copy — inventory list is SHARED between original and clone
original = Monster("Dragon", 1000, ["sword", "shield"])
shallow = copy.copy(original)
shallow.inventory.append("potion")
print(original.inventory)  # ['sword', 'shield', 'potion'] — Bug!

# Deep copy — inventory list is fully independent
deep = copy.deepcopy(original)
deep.inventory.append("ring")
print(original.inventory)  # ['sword', 'shield', 'potion'] — Unaffected
```

### Best implementation (Prototype with Registry)
A clean Prototype interface with a registry for pre-configured templates.
...
class Monster(Prototype):
    def __init__(self, name: str, health: int, attack: int, abilities: list):
        self.name = name
        self.health = health
        self.attack = attack
...
class MonsterRegistry:
    def __init__(self):
        self._cache: Dict[str, Monster] = {}

    def register(self, key: str, prototype: Monster):
        self._cache[key] = prototype

    def get(self, key: str) -> Monster:
        if key not in self._cache:
            raise KeyError(f"Unknown monster type: {key}")
        return self._cache[key].clone()

# Usage
def main():
    registry = MonsterRegistry()
    registry.register("dragon", Monster("Dragon", 1000, 50, ["fire_breath", "fly"]))
    registry.register("goblin", Monster("Goblin", 100, 10, ["sneak"]))

    # Instantly get fully-formed clones
    mob1 = registry.get("dragon")
    mob2 = registry.get("dragon")
    mob2.health = 800

    print(mob1)  # Dragon | HP: 1000 | ATK: 50
    print(mob2)  # Dragon | HP: 800 | ATK: 50
    print(mob1 is mob2)  # False — independent objects
```
...
4. **Spreadsheet Cells:** Copying a cell in Excel/Google Sheets clones its formatting, formula, and style. The new cell is independent but starts identical.
5. **CI/CD Pipeline Templates:** A base pipeline configuration is cloned and customized per project/branch. Jenkins, GitLab CI, and GitHub Actions all use template-based approaches that are conceptually Prototype patterns.
...
- **Q: When is Prototype preferred over using `new`?**
- A: When object construction is expensive (requires DB calls, network requests, or complex calculations), and you need multiple similar objects with minor variations.
...
- **Q: How does the Prototype Registry (Cache) pattern improve performance in a game engine?**
- A: Pre-configured prototypes (e.g., enemy types, weapons) are loaded once during initialization. At runtime, spawning a new enemy is a `clone()` call instead of re-reading mesh data, textures, and AI configurations from disk. This can reduce spawn time from milliseconds to microseconds.
...
- **Q: What are the problems with Java's `Cloneable` interface and how do you avoid them?**
- A: Java’s `Cloneable` is a marker interface with no methods — the actual `clone()` method is on `Object` and is `protected`. It performs only shallow copies by default, throws `CloneNotSupportedException` (a checked exception), and has no way to enforce deep copy contracts. Prefer copy constructors or the `Prototype` interface pattern demonstrated above.
...
- **Q: In a distributed system, how would you implement a Prototype Registry that is shared across multiple service instances?**
- A: Use an external store (Redis, Memcached, or a database) as the registry. Prototypes are serialized and stored centrally. Each service instance deserializes the prototype on demand, effectively performing a deep copy over the network. This is how distributed caching systems (e.g., Hazelcast) implement near-cache with prototype-like semantics.
