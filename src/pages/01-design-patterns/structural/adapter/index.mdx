...
# 3. Adapter (Translates Target calls to Adaptee calls)
class MediaAdapter(MediaPlayer):
    ...
    def play(self, audio_type: str, file_name: str) -> None:
        dispatch = {
            "vlc": self._advanced_player.play_vlc,
            "mp4": self._advanced_player.play_mp4,
        }
        handler = dispatch.get(audio_type.lower())
        if handler:
            handler(file_name)
        else:
...
player.play("vlc", "lecture.vlc")    # Playing vlc file: lecture.vlc
```
...
4. **Payment Gateway Adapters:** Wrapping Stripe, PayPal, and Razorpay SDKs behind a unified `PaymentGateway` interface so your application code stays payment-provider-agnostic.
5. **Logging Libraries:** SLF4J in Java acts as a Facade + Adapter. It provides a unified logging API while adapting to Logback, Log4j, or java.util.logging under the hood.
...
- **Q: Give a simple real-world analogy for the Adapter pattern.**
  - **A:** A power plug adapter that lets a US charger plug into a European wall socket. The adapter translates between two incompatible physical interfaces.
...
- **Q: Can an Adapter work with multiple Adaptees simultaneously?**
  - **A:** Yes. An Adapter can compose multiple Adaptees and route calls to the appropriate one based on the request type. This is sometimes called a "Multi-Adapter" or "Universal Adapter" — common in payment gateways that wrap Stripe, PayPal, and Razorpay.
...
- **Q: When would you prefer the Adapter pattern over refactoring the Adaptee's interface?**
  - **A:** When you don't own the Adaptee's source code (3rd-party library), when the Adaptee is used by other clients who depend on its current interface, or when you need to adapt multiple incompatible Adaptees to a single Target interface.

### Staff Engineer
- **Q: How does the Adapter pattern relate to the Anti-Corruption Layer in Domain-Driven Design (DDD)?**
  - **A:** The Anti-Corruption Layer (ACL) is essentially a set of Adapters at the boundary between two bounded contexts. It prevents the domain model of one context from being "corrupted" by the model of another. The ACL translates between external models (legacy systems, 3rd-party APIs) and your clean internal domain model, exactly like an Adapter translates between incompatible interfaces.

- **Q: In a microservices migration, how do Adapters help with the Strangler Fig pattern?**
  - **A:** When gradually replacing a monolith, Adapters sit between the new microservice and the old monolith's API. The Adapter translates the old API's format into the new service's expected format. As the migration progresses, you move more logic to the new service and eventually remove the Adapter. This allows incremental migration without a risky "big bang" rewrite.
