...
In Python, we use the `abc` module for abstract base classes. To simulate the `final` keyword, we can use the `@typing.final` decorator (since Python 3.8).
...
class DataMiner(ABC):
    ...
    def mine_data(self, path: str) -> None:
        """The Template Method: defines the invariant sequence of steps."""
        self.open_file(path)
        self.extract_data()
        self.parse_data()
        self.analyze_data()
    ...
    # Invariant methods (common implementation)
    def open_file(self, path: str) -> None:
        print(f"Opening file: {path}")

    def analyze_data(self) -> None:
        print("Analyzing data generically.")

    def send_report(self) -> None:
        print("Sending report.")

    # Variant methods (abstract steps to be defined by subclasses)
    @abstractmethod
...
class PDFDataMiner(DataMiner):
    ...
    def parse_data(self) -> None:
        print("Parsing PDF internal object hierarchy.")


class CSVDataMiner(DataMiner):
    def extract_data(self) -> None:
        print("Reading raw CSV bytes.")

    def parse_data(self) -> None:
        print("Parsing CSV cells into structured rows.")

    def clean_up(self) -> None:
        print("Releasing file pointer handles.")
...
if __name__ == "__main__":
    ...
    pdf_miner.mine_data("financial_report.pdf")

    print("\n--- CSV Miner ---")
    csv_miner = CSVDataMiner()
    csv_miner.mine_data("transactions.csv")
```
...
4. **Testing Frameworks:** In JUnit or Pytest, the base execution model (setup → test run → teardown) is defined by the runner, while developers override `setUp()` or write test methods.
5. **Python standard library:** The `unittest.TestCase` class executes a test suite by template-calling `setUp()`, running the test method, and then executing `tearDown()`.
...
- **Q: How does the Template Method pattern enforce code reuse?**
  - **A:** By centralizing the common steps and flow of an algorithm in an abstract base class, so subclasses only need to implement the steps that vary, reducing duplication.
...
- **Q: What is the "Hollywood Principle" and how does it relate to this pattern?**
  - **A:** The principle is: "Don't call us, we'll call you." In the Template Method pattern, the base class controls the high-level flow and calls subclass implementations when needed. Subclasses do not dictate the flow, they are simply called.
...
- **Q: How can the Template Method violate the Liskov Substitution Principle (LSP) if not designed carefully?**
  - **A:** If a subclass implements one of the abstract steps by throwing an `UnsupportedOperationException`, it changes the postconditions and assumptions of the base class's template algorithm. Subclasses must adhere to the contract of the steps defined in the abstract base.

### Staff Engineer
- **Q: How do modern web frameworks (like Spring MVC interceptors or Express middlewares) implement the Template Method concept dynamically without classical inheritance?**
  - **A:** They use the **Interceptor Pattern** or **Chain of Responsibility** paired with delegation. Instead of extending a base class to fill in hooks, developers write standalone middleware or interceptors that register callbacks. The framework runs a loop that executes these dynamically assembled callbacks around the handler, avoiding inheritance completely and maintaining flexibility.
- **Q: Inheritance is often considered a design smell due to tight coupling. How can you refactor a legacy Template Method pattern to a composition-based design?**
  - **A:** You can refactor it by replacing inheritance with the **Strategy Pattern** or passing functions/callbacks. Define the main workflow context that accepts a list of strategies or lambda functions for the variable steps. The context executes the invariant steps and invokes the strategy implementations or delegates for the variant parts, decoupling the components.
...
- [Liskov Substitution Principle (LSP)](../../../03-lld/solid/liskov-substitution-principle)
