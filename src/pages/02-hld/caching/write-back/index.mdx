...
graph TD
    ...
    subgraph Asynchronous Flush Path
        F_Trigger[Time/Size Trigger] --> F_Fetch[Drain Dirty Records]
        F_Fetch --> F_Coalesce[Coalesce / Merge Duplicates]
        F_Coalesce --> F_DB[(Batch Write to DB)]
        F_DB -- Success --> F_Clean[Clear Dirty Flags]
        F_DB -- Fail --> F_Retry[Retry / Move to DLQ]
    end
    ...
    classDef alert fill:#f8d7da,stroke:#dc3545,stroke-width:2px,color:#721c24;

    class W_Start,W_Ack startEnd;
    class W_Cache,W_Mark,F_Fetch,F_Coalesce,F_Clean cache;
    class F_DB db;
    class F_Trigger trigger;
    class F_Retry alert;
```
