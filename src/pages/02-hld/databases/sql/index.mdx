...
```mermaid
flowchart TD
    subgraph Analysis ["1. Analysis & Optimization"]
        SQL["SQL Query"] --> Parser["Parser (Syntax Check)"]
        Parser --> Binder["Binder (Schema Resolution)"]
        Binder --> Optimiser["Query Optimiser"]
    end

    subgraph Execution ["2. Execution & Storage"]
    ...
    class SQL,Result startEnd;
    class Parser,Binder,Optimiser,Plan,Executor process;
    class Storage db;
```
